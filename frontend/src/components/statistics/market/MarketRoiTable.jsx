import { formatPercent } from "../../../utils/statisticsFormatter";

export default function MarketRoiTable({ data = [] }) {
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h3 className="mb-4 text-lg font-semibold text-slate-900">Variation du ROI market</h3>
          ...
          <tbody className="divide-y divide-slate-100">
            {data.map((row) => (
              <tr key={row.period} className="hover:bg-slate-50">
                ...
                <td className="px-3 py-3 text-slate-900">{formatPercent(row.value)}</td>
              </tr>
            ))}
          </tbody>
        </table>
