...
export default function ResalePotentialTable({ data = [] }) {
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h3 className="mb-4 text-lg font-semibold text-slate-900">Potentiel de revente</h3>
        ...
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-3 py-3">ROI potentiel</th>
            </tr>
          </thead>

          <tbody className="divide-y divide-slate-100">
            {data.map((row) => {
              const potentialClass = Number(row.potentielEur) >= 0 ? "text-emerald-700" : "text-rose-700";
              ...
              return (
                <tr key={row.id} className="hover:bg-slate-50">
                  ...
                  <td className={`px-3 py-3 font-medium ${potentialClass}`}>{formatCurrency(row.potentielEur)}</td>
                  <td className={`px-3 py-3 ${potentialClass}`}>{formatPercent(row.roiPotentiel)}</td>
                </tr>
              );
            })}
          </tbody>
