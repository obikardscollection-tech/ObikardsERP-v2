...
export default function TopRoiTable({ data = [] }) {
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h3 className="mb-4 text-lg font-semibold text-slate-900">Top ROI</h3>
          ...
          <tbody className="divide-y divide-slate-100">
            {data.map((row) => (
              <tr key={`${row.sku}-${row.title}`} className="hover:bg-slate-50">
                ...
                <td className="px-3 py-3 font-medium text-emerald-700">{formatPercent(row.roi)}</td>
